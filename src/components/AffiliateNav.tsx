...
export default function AffiliateNav({ profileInitial, onSignOut }: Props) {
  ...
  return (
    <>
      <style>{`
        .a2-nav-links { margin-left: 2rem; display: flex; gap: 0.25rem; overflow-x: auto; scrollbar-width: none; flex-shrink: 0; }
        .a2-nav-links::-webkit-scrollbar { display: none; }
        .a2-nav-right { margin-left: auto; display: flex; align-items: center; gap: 0.75rem; flex-shrink: 0; }
        .a2-hamburger { display: none; background: none; border: none; cursor: pointer; padding: 0.25rem; }
        .a2-mobile-link { display: block; padding: 0.85rem 1.25rem; font-size: 14px; font-weight: 500; color: #3a3a3a; text-decoration: none; border-bottom: 1px solid #f2f0ec; }
        .a2-mobile-link.active { color: #0d0d0d; background: #f9f8f6; font-weight: 600; }
        .a2-mobile-cta { display: block; margin: 0.75rem 1.25rem; background: #0d0d0d; color: #ffffff; font-size: 13px; font-weight: 600; padding: 0.7rem 1.25rem; border-radius: 4px; text-decoration: none; text-align: center; }
        @media (max-width: 768px) {
          .a2-nav-links { display: none; }
          .a2-nav-right { display: none; }
          .a2-hamburger { display: flex; margin-left: auto; }
        }
      `}</style>
          ...
          {open && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#ffffff', borderBottom: '1px solid #e8e6e2', boxShadow: '0 8px 24px rgba(0,0,0,0.08)', paddingBottom: '0.75rem' }}>
              {NAV_LINKS.map(n => {
                const active = pathname === n.href
                return (
                  <Link key={n.label} href={n.href} className={`a2-mobile-link${active ? ' active' : ''}`} onClick={() => setOpen(false)}>
                    {n.label}
                  </Link>
                )
              })}
