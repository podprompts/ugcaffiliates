...
function VendorDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: prods }, { data: convs }, { data: apps }] = await Promise.all([
        ...
        ids.length > 0
          ? supabase.from('affiliate_applications').select('id, applied_at, profiles!affiliate_id(full_name), products(title)').eq('status', 'pending').in('product_id', ids).order('applied_at', { ascending: false }).limit(10)
  ...
  async function handleReapply() {
    ...
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) return
    const res = await fetch('/api/vendor/reapply', {
      method: 'POST',
      headers: { Authorization: `Bearer ${session.access_token}` },
    })
    const data = await res.json()
    if (data.ok) {
      setProfile((prev: any) => ({ ...prev, vendor_status: 'pending_approval', reapply_count: (prev.reapply_count ?? 0) + 1 }))
    }
    setReapplying(false)
  }
  ...
  const isPending = profile?.vendor_status === 'pending_approval'
      ...
      <div className="v-content">
        <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          {isApproved ? (
            ...
            <span style={{ fontSize: '13px', color: '#aaa', border: '1px solid #e8e6e2', padding: '0.5rem 1.25rem', borderRadius: '3px', whiteSpace: 'nowrap', cursor: 'not-allowed' }}>+ List a new product</span>
          )}
        </div>
        ...
        {isPending && (
          <div style={{ background: '#fef9ec', border: '1px solid #fde68a', borderRadius: '4px', padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
            <div style={{ fontSize: '14px', fontWeight: 600, color: '#92400e', marginBottom: '0.4rem' }}>Your vendor application is under review</div>
            <p style={{ fontSize: '13px', color: '#92400e', margin: 0, lineHeight: 1.6 }}>
              Our team is reviewing your application — usually within 24 hours. You'll receive an email once a decision is made. You cannot list products until approved.
            </p>
          </div>
        )}
        ...
        {/* Rejected banner */}
        {isRejected && (
          <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '4px', padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
            <div style={{ fontSize: '14px', fontWeight: 600, color: '#dc2626', marginBottom: '0.4rem' }}>Your vendor application was not approved</div>
            {profile.rejection_note && (
              <p style={{ fontSize: '13px', color: '#dc2626', margin: '0 0 0.75rem', lineHeight: 1.6 }}>
                <strong>Reason:</strong> {profile.rejection_note}
              </p>
            )}
            {(profile.reapply_count ?? 0) < 1 ? (
              <button onClick={handleReapply} disabled={reapplying}
                style={{ fontSize: '13px', fontWeight: 600, color: '#ffffff', background: '#dc2626', border: 'none', padding: '0.5rem 1.25rem', borderRadius: '3px', cursor: reapplying ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: reapplying ? 0.6 : 1 }}>
                {reapplying ? 'Submitting…' : 'Reapply →'}
              </button>
            ) : (
              <p style={{ fontSize: '13px', color: '#888', margin: 0 }}>You have used your one reapplication opportunity. Contact us if you believe this is an error.</p>
            )}
          </div>
        )}
