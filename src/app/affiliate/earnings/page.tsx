...
export default function AffiliateEarningsPage() {
  ...
  const statusLabel: Record<string, string> = {
    pending: 'Pending', approved: 'Approved — awaiting payment', paid: 'Paid', disputed: 'Disputed'
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f8f6', fontFamily: 'var(--font-dm-sans), sans-serif' }}>
      ...
      <nav style={{ background: '#ffffff', borderBottom: '1px solid #e8e6e2', padding: '0 2rem', display: 'flex', alignItems: 'center', height: '60px', position: 'sticky' as const, top: 0, zIndex: 50 }}>
        ...
        <div style={{ marginLeft: '2rem', display: 'flex', gap: '0.25rem' }}>
          {[
            ...
            { label: 'Earnings',  href: '/affiliate/earnings', active: true },
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '2.5rem 2rem' }}>
        ...
        {/* Summary cards */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1px', background: '#e8e6e2', marginBottom: '2rem' }}>
          {[
          ...
          {conversions.length === 0 ? (
            ...
            <>
              ...
              {conversions.map(c => {
                ...
                return (
                  <div key={c.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '1rem 1.5rem', borderBottom: '1px solid #e8e6e2', alignItems: 'center' }}>
                    <div>
        ...
        {payouts.length > 0 && (
          <div style={{ background: '#ffffff', border: '1px solid #e8e6e2', borderRadius: '4px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', padding: '0.75rem 1.5rem', borderBottom: '1px solid #e8e6e2', background: '#f9f8f6' }}>
              {['Period', 'Amount', 'Status', 'Date paid'].map(h => (
                <div key={h} style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#888' }}>{h}</div>
              ))}
            </div>
            {payouts.map(p => (
              <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', padding: '1rem 1.5rem', borderBottom: '1px solid #e8e6e2', alignItems: 'center' }}>
                ...
                <div style={{ fontSize: '12px', color: p.status === 'paid' ? '#16a34a' : '#888', fontWeight: 500, textTransform: 'capitalize' as const }}>{p.status}</div>
