              ...
              return (
                <div key={link.id} style={{ background: '#ffffff', padding: '1.5rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: '#f9f8f6', padding: '0.75rem 1rem', borderRadius: '3px', marginBottom: '1rem' }}>
                    <code style={{ flex: 1, fontSize: '12.5px', color: '#0d0d0d', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{shortUrl}</code>
                    <button onClick={() => copyLink(shortUrl, link.id)} style={{ fontSize: '12px', fontWeight: 600, color: copied === link.id ? '#16a34a' : '#0d0d0d', background: '#ffffff', border: '1px solid #e8e6e2', padding: '0.35rem 0.75rem', borderRadius: '3px', cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0, whiteSpace: 'nowrap' }}>
                      {copied === link.id ? 'Copied!' : 'Copy link'}
                    </button>
                  </div>
                  <div className="al-link-stats">
                    {[
                      { label: 'Clicks',      value: (link.total_clicks ?? 0).toLocaleString() },
                      { label: 'Conversions', value: (link.total_conversions ?? 0).toString() },
                      { label: 'Conv. rate',  value: `${convRate}%` },
                      { label: 'Earned',      value: `$${(link.total_earned ?? 0).toFixed(2)}` },
                    ].map(s => (
                      <div key={s.label}>
