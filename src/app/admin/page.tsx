  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f8f6', fontFamily: 'var(--font-dm-sans), sans-serif' }}>
      ...
      <nav style={{ background: '#0d0d0d', padding: '0 2rem', display: 'flex', alignItems: 'center', height: '56px', position: 'sticky' as const, top: 0, zIndex: 50 }}>
        ...
        <div style={{ marginLeft: '2rem', display: 'flex', gap: '0.25rem' }}>
          {[
            ...
            <Link key={n.label} href={n.href} style={{ fontSize: '13px', fontWeight: 500, padding: '0.4rem 0.85rem', borderRadius: '4px', textDecoration: 'none', background: n.active ? 'rgba(255,255,255,0.1)' : 'transparent', color: n.active ? '#ffffff' : 'rgba(255,255,255,0.5)' }}>
              {n.label}
            </Link>
          ))}
        </div>
        <button
          ...
          style={{ marginLeft: 'auto', fontSize: '12px', color: 'rgba(255,255,255,0.4)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
        >
          Sign out
        </button>
      </nav>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2.5rem 2rem' }}>
        ...
        {/* Stats */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '1px', background: '#e8e6e2', marginBottom: '2rem' }}>
          {[
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ background: '#ffffff', border: '1px solid #e8e6e2', borderRadius: '4px', padding: '1.75rem' }}>
            ...
            {recentUsers.map(u => (
              <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.6rem 0', borderBottom: '1px solid #f2f0ec' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 600, color: roleColor[u.role] ?? '#888', background: `${roleColor[u.role]}15`, padding: '0.15rem 0.5rem', borderRadius: '100px', textTransform: 'capitalize' as const }}>
                  {u.role}
                </span>
                {u.role === 'vendor' && (
                  <span style={{ fontSize: '10px', color: u.stripe_onboarded ? '#16a34a' : '#888' }}>
          ...
          <div style={{ background: '#ffffff', border: '1px solid #e8e6e2', borderRadius: '4px', padding: '1.75rem' }}>
            ...
            {recentConversions.length === 0 ? (
              <div style={{ fontSize: '13px', color: '#888', textAlign: 'center' as const, padding: '2rem 0' }}>No conversions yet</div>
              ...
              return (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.6rem 0', borderBottom: '1px solid #f2f0ec' }}>
                  ...
                  <div style={{ textAlign: 'right' as const }}>
                    ...
                    <div style={{ fontSize: '11px', color: statusColor[c.status] ?? '#888', fontWeight: 500, textTransform: 'capitalize' as const }}>{c.status}</div>
                  </div>
                </div>
              )
            })}
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e8e6e2', borderRadius: '4px', padding: '1.75rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', gap: '0' }}>
            {['Product', 'Vendor', 'Commission', 'Sales', 'Status'].map(h => (
              <div key={h} style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#888', paddingBottom: '0.75rem' }}>{h}</div>
            ))}
          </div>
          {recentProducts.map(p => {
            const vendor = p.profiles as any
            return (
              <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '0.75rem 0', borderTop: '1px solid #f2f0ec', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: statusColor[p.status] ?? '#888' }} />
                  ...
                  <button
                    ...
                    style={{ fontSize: '11px', color: '#888', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', fontFamily: 'inherit', marginLeft: '0.25rem' }}
                  >
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e8e6e2', borderRadius: '4px', padding: '1.75rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '1px', background: '#e8e6e2' }}>
            {platformRules.map(rule => (
              <div key={rule.rule_key} style={{ background: '#ffffff', padding: '0.85rem 1rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
