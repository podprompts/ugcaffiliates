...
function MarketplacePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      let query = supabase
        .from('products')
        .select('id, slug, title, description, price, commission_rate, category, image_url, images, total_conversions, auto_approve, profiles!vendor_id(full_name, business_name)')
        .eq('status', 'active')
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#ffffff', fontFamily: 'var(--font-dm-sans), sans-serif' }}>
      <style>{`
        ...
        .lock-badge { position: absolute; bottom: 8px; left: 8px; background: rgba(13,13,13,0.82); color: #fff; font-size: 10.5px; font-weight: 600; padding: 0.25rem 0.6rem; border-radius: 2px; display: flex; align-items: center; gap: 0.35rem; backdrop-filter: blur(4px); }
        .lock-badge a { color: #ffffff; text-decoration: underline; text-underline-offset: 2px; font-size: 10.5px; font-weight: 600; }
                ...
                return (
                  <Link key={p.id} href={`/marketplace/${slug}`} className="product-card">
                    <div style={{ position: 'relative' as const, width: '100%', aspectRatio: '1', background: '#f2f0ec', borderRadius: '2px', marginBottom: '0.75rem', overflow: 'hidden' }}>
                      ...
                      {user ? (
                        ...
                        <div className="lock-badge">
                          <svg width="10" height="10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><rect x="3" y="11" width="18" height="11" rx="2"/><path strokeLinecap="round" d="M7 11V7a5 5 0 0110 0v4"/></svg>
                          <Link href="/signup" onClick={e => e.stopPropagation()}>Sign up to see</Link>
                        </div>
                      )}
                    </div>

                    <div style={{ fontFamily: 'var(--font-cormorant), serif', fontSize: '11px', fontWeight: 500, color: '#888', marginBottom: '0.2rem', letterSpacing: '0.04em', textTransform: 'uppercase' as const }}>{displayName}</div>
                    ...
                    {user ? (
                      ...
                      <div style={{ fontSize: '12px', color: '#888', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.3rem', flexWrap: 'wrap' as const }}>
                        Earn up to <span style={{ filter: 'blur(5px)', userSelect: 'none' as const, color: '#0d0d0d', fontWeight: 600 }}>$••.••</span> per sale
                      </div>
                    )}
